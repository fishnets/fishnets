...
Examples for steepness for individual species gave reasonable results for New Zealand stocks, but gave questionablke results in the case of Atlantic Cod and especially skipjack tuna. Since the steepness nodes did not produce high predictive power in jackknife tests, the accuracy of the estiamtes in these examples is unclear. 

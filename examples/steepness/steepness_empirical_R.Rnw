...
R_net <- Fishnet(
  ...
  mean_R_z = Brter(tr_R(mean_R_z) ~  habit + 
                      log(linf) + 
                      log(k) + 
                      log(m) + 
                      log(fecundity) +
...
formulae <- vector(,6)
formulae[1] <- paste('tr_R(mean_R_z) ~ ',
                     paste(covars[c(3,4)],collapse = ' + '))
formulae[2] <- paste('tr_R(mean_R_z) ~ ',
                     paste(covars[c(3,4,7)],collapse = ' + '))
formulae[3] <- paste('tr_R(mean_R_z) ~ ',
                     paste(covars[c(2,4,7)],collapse = ' + '))
formulae[4] <- paste('tr_R(mean_R_z) ~ ',
                     paste(covars[c(2,3,4,7)],collapse = ' + '))
formulae[5] <- paste('tr_R(mean_R_z) ~ ',
                     paste(covars[c(1,2,3,4,5,7)],collapse = ' + '))
formulae[6] <- paste('tr_R(mean_R_z) ~ ',
...
R_net$nodes$mean_R_z  = Bayser(tr_R(mean_R_z) ~ f(family,model='iid') + 
                                   f(habit,model='iid') + 
                                   log(linf) + 
                                   log(k) + 
                                   log(m) + 
                                   ...
                                   log(m)*log(recsigma) + 
                                   log(trophic) + 
                                   log(depthmax),transform = tr_R_inv)

R_net$nodes$mean_R_z$fit(testset)
...
formulae[6] <- paste('tr_R(mean_R_z) ~ ',
                     ...
                     "f(habit,model='iid') +",
                     paste(covars[c(2,3,4,7)],collapse = ' + '))
formulae[7] <- paste('tr_R(mean_R_z) ~ ',
                     "f(family,model='iid') +",
                     paste(covars,collapse = ' + '))
formulae[8] <- paste('tr_R(mean_R_z) ~ ',
                     "f(family,model='iid') +",
                     "f(habit,model='iid') +",
                     "log(m)*log(recsigma) +",
                     paste(covars,collapse = ' + '))
...
tune <- R_net$nodes$mean_R_z$tune(testset,formulae,folds = sum(!is.na(testset[['mean_R_z']])))
# 
steep_cv_bayes <- jackknife_cv(testset,R_net,'mean_R_z')
@
...
The snapper predictions from the Beverton-Holt steepness node seem to provide sensible estimates (\autoref{fig:snapper}), with a distribution centered around \Sexpr{mean(preds.snapper$mean_R_z)}, which is reasonably close to estiamtes used in the assessments for SNA1 and SNA7. However, the density near 1 is very low, suggesting that the SNA7 assumption of z=1 is not consistent with life-history traits.

\subsubsection{Hoki}

Hoki life-history parameters were extracted from the 2012 assessment of HOK1. The steepness value in the assessment is 0.75, reduced from 0.9 (used prior to 2008). Estiamtes from the steepness node suggest that even 0.75 is at the upper limit of what life history correlates would suggest (\autoref{fig:hoki}).
